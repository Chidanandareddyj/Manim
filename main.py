...
class IMUIntro(ThreeDScene):
    ...
    def construct(self):
        ...
        # Axis labels (positioned clearly)
        ...
        # === POSITION ACCELEROMETER CENTERED ON LEFT ===
        accel_group = VGroup(cube, axes_group)
        accel_group.move_to(LEFT * 3.5)  # Centered on left half
        
        # === ACCELERATION GRAPHS (RIGHT SIDE - CLEAN) ===
        graph_y_positions = [2.0, 0.2, -1.6]
        graph_colors = ["#ef4444", "#22c55e", "#3b82f6"]
        graph_names = ["X", "Y", "Z"]
        
        # Synthetic data - MUCH LESS NOISE
        num_samples = 400
        time_range = np.linspace(0, 5, num_samples)
        
        # At rest: Z = -1g, X/Y ≈ 0 (minimal noise)
        np.random.seed(42)  # Reproducible
        z_data = np.full(num_samples, -1.0) + 0.008 * np.random.randn(num_samples)
        x_data = 0.005 * np.random.randn(num_samples)
        y_data = 0.005 * np.random.randn(num_samples)
        ...
        for i, (y_pos, color, name, data) in enumerate(zip(graph_y_positions, graph_colors, graph_names, data_sets)):
            ...
            line = VMobject(color=color, stroke_width=2.0)
            line.set_points_smoothly(points[::2])  # Smooth, fewer points
        ...
        # [3.5-5s] Bring in graphs
        self.play(
            *[FadeIn(g) for g in graph_groups],
            run_time=1.5
        )
        
        # [5-7.5s] Animate signal lines
        self.play(
        ...
        narration.to_edge(DOWN, buff=0.4)
        self.add_fixed_in_frame_mobjects(narration)
        
        self.play(FadeIn(narration, shift=UP * 0.2), run_time=1.5)
        self.wait(1.5)
